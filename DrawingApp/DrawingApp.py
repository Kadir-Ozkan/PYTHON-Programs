...
class FreeHandDrawing(tk.Tk):
    def __init__(self):
        ...
        self.color = "Black"
        self.thickness = 1
        self.tag = ["tag", "0"] #for undoing (to remember what was to be deleted in what order)
    ...
    def _create_widgets(self):
        ...
        colorList = ttk.Combobox(topframe, textvariable = self.col_select, values =['Black', 'Green', 'Brown', 'Red', 'Yellow'], state = "readonly", width = 10)
        colorList.current(0)
        ...
        tList = ttk.Combobox(topframe, textvariable = self.t_select, values = [1, 2, 3, 4, 5, 6, 7], state = "readonly", width = 3)
        tList.current(0)
